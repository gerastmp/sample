...
make_task_def(){
	task_template='[
		{
			"name": "django-sample-webapp",
			"image": "%s.dkr.ecr.eu-west-1.amazonaws.com/django-sample-webapp:%s",
			"essential": true,
			"memory": 200,
			"cpu": 10,
			"portMappings": [
...
push_ecr_image(){
	...
	docker push $AWS_ACCOUNT_ID.dkr.ecr.eu-west-1.amazonaws.com/django-sample-webapp:$CIRCLE_SHA1
}
